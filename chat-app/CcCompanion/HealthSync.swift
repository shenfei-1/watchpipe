//  HealthSync.swift — HealthKit 采集（珩 2026-09-11，1.3 build 242，从 WatchPipe 搬来）
//  观察者查询 + 后台投递（enableBackgroundDelivery）+ 锚点增量查询。HealthKit 回调在后台线程，
//  一律 Task { @MainActor } 回主线程再碰 Outbox / 日志（这个工程默认 MainActor 隔离）。
...
final class HealthSync {
    ...
    private var bgDeliveryOK: Set<String> = []

    static let perMin = HKUnit.count().unitDivided(by: .minute())
    static let quantities: [HealthQuantitySpec] = [
    ...
    func requestAuthorization(_ done: @escaping (Bool) -> Void) {
        guard isAvailable else { HealthLog.shared.add("这台设备没有 HealthKit"); done(false); return }
        store.requestAuthorization(toShare: nil, read: readTypes) { ok, err in
            let msg = err?.localizedDescription ?? ""
            Task { @MainActor in
                HealthLog.shared.add(ok ? "健康授权完成" : "健康授权失败: \(msg)")
                UserDefaults.standard.set(ok, forKey: HealthStatus.authorizedKey)
    ...
    func startIfAuthorized() {
        guard isAvailable, HealthStatus.authorized, !observersRegistered else { return }
        observersRegistered = true
        let total = allSampleTypes.count
        for (name, type) in allSampleTypes {
            let oq = HKObserverQuery(sampleType: type, predicate: nil) { _, completion, err in
                let msg = err?.localizedDescription
                Task { @MainActor in
                    if let msg { HealthLog.shared.add("\(name) 观察者出错: \(msg)"); completion(); return }
                    self.sync(name: name, type: type) {
                        Uploader.shared.flush(reason: "后台投递 \(name)")
                        completion()
                    }
                }
            }
            ...
            store.enableBackgroundDelivery(for: type, frequency: .immediate) { ok, err in
                let msg = err?.localizedDescription ?? ""
                Task { @MainActor in
                    if ok { self.bgDeliveryOK.insert(name) } else { self.bgDeliveryOK.remove(name) }
                    UserDefaults.standard.set(self.bgDeliveryOK.count == total, forKey: HealthStatus.bgDeliveryKey)
                    HealthLog.shared.add(ok ? "\(name) 后台投递已开" : "\(name) 后台投递未开: \(msg)")
                }
            }
        }
        HealthLog.shared.add("已注册 \(total) 个类型的观察者")
    }

    // MARK: 同步
    func syncAll(reason: String, done: @escaping () -> Void) {
        guard isAvailable, HealthStatus.authorized else { done(); return }
        let g = DispatchGroup()
        for (name, type) in allSampleTypes { g.enter(); sync(name: name, type: type) { g.leave() } }
        g.notify(queue: .main) {
            MainActor.assumeIsolated {
                Uploader.shared.flush(reason: reason)
                done()
            }
        }
    }

    private func anchorKey(_ name: String) -> String { "hp.anchor.\(name)" }
    ...
    func sync(name: String, type: HKSampleType, done: @escaping () -> Void) {
        ...
        let query = HKAnchoredObjectQuery(type: type, predicate: predicate, anchor: anchor, limit: HKObjectQueryNoLimit) { _, samples, _, newAnchor, err in
            let msg = err?.localizedDescription
            Task { @MainActor in
                if let msg { HealthLog.shared.add("\(name) 查询出错: \(msg)"); done(); return }
                ...
                if !out.isEmpty { HealthLog.shared.add("\(name) 入队 \(out.count) 条") }
                done()
            }
        }
        store.execute(query)
    ...
    private func convert(name: String, samples: [HKSample]) -> [HealthSample] {
        if name == "sleep" {
            return samples.compactMap { s in
                ...
                return HealthSample(type: "sleep", start: iso.string(from: c.startDate), end: iso.string(from: c.endDate), value: v, unit: nil)
            }
        }
        ...
        return samples.compactMap { s in
            ...
            return HealthSample(type: spec.name, start: iso.string(from: qs.startDate), end: iso.string(from: qs.endDate), value: str, unit: spec.label)
        }
    }
}
